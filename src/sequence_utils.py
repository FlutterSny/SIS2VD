...
def scan_sequence(directory: Path, pattern_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    Searches for ALL files with the same naming scheme (prefix + extension + any number),
    then detects the actual sequence from all matching files. This handles both
    counting up and counting down scenarios, and works regardless of which
    specific file was used to extract the pattern.
    ...
    Returns:
        Dictionary with sequence info or None if no files found:
        {
            ...
            'padding': 4                     # Detected padding from actual files
        }
    """
    prefix = pattern_info['prefix']
    extension = pattern_info['extension']
    
    # Build regex pattern to match files with same prefix and extension, any number
    # e.g., Shot_1_(\d+)\.png - matches any number of digits
    regex_pattern = re.escape(prefix) + r'(\d+)' + re.escape(extension) + r'$'
    ...
    # Use the most common padding, or the one from pattern_info if consistent
    if len(paddings) == 1:
        detected_padding = paddings.pop()
    else:
        detected_padding = pattern_info['padding']
